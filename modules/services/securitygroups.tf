...
resource "aws_security_group" "allow_redis_in" {
  ...
  ingress {
    ...
    to_port     = 6379
    protocol    = "TCP"
    cidr_blocks = [data.aws_subnet.private_subnet.cidr_block,data.aws_subnet.private2_subnet.cidr_block]
  }
  egress {
    from_port   = 6379
    to_port     = 6379
...
resource aws_security_group "allow_rds_in" {
  ...
  ingress {
    ...
    to_port     = 3306
    protocol    = "tcp"
    cidr_blocks = [data.aws_subnet.private_subnet.cidr_block,data.aws_subnet.private2_subnet.cidr_block,data.aws_subnet.public_subnet.cidr_block, data.aws_subnet.public2_subnet.cidr_block]
  }
  egress {
    from_port   = 3306
    to_port     = 3306
